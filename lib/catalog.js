...
const COPY = {
  ...
  /* Boat services are priced per foot of hull. Rates live in Supabase under
     service_id 'boat-detail'; every job is still quoted personally. */
  boat: [
    { id: 'wash-ft',        name: 'Wash' },
    { id: 'interior-ft',    name: 'Interior detail' },
    { id: 'wax-ft',         name: 'Wax' },
    { id: 'sealant-ft',     name: 'Sealant' },
    { id: 'oxidation-ft',   name: 'Oxidation removal' },
    { id: 'ceramic-ft',     name: 'Ceramic coating' },
    { id: 'maintenance-ft', name: 'Maintenance wash' },
  ],
};
...
const FALLBACK = {
  ...
  boat: [
    { id: 'wash-ft',        name: 'Wash',              perFt: 10 },
    { id: 'interior-ft',    name: 'Interior detail',   perFt: 8 },
    { id: 'wax-ft',         name: 'Wax',               perFt: 5 },
    { id: 'sealant-ft',     name: 'Sealant',           perFt: 25 },
    { id: 'oxidation-ft',   name: 'Oxidation removal', perFt: 20 },
    { id: 'ceramic-ft',     name: 'Ceramic coating',   perFt: 100 },
    { id: 'maintenance-ft', name: 'Maintenance wash',  perFt: 6 },
  ],
  addons: [
    { id: 'clay-bar',   name: 'Clay Bar + Hand Wax',   price: 100, minutes: 60, note: 'Deep paint decontamination plus hand wax',
...
export async function getCatalog() {
  ...
  try {
    ...
    const boatRows = rowsFor('boat-detail');
    const boat = COPY.boat
      .map((b) => {
        const row = boatRows.find((r) => r.size_id === b.id);
        return row ? { ...b, perFt: +row.price } : null;
      })
      .filter(Boolean);

    return { currency: COPY.currency, sizes: COPY.sizes, detail, maintenance, plan, addons, boat };
  } catch (err) {
